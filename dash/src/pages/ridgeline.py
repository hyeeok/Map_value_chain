...
from src.lib.default import cal_ratio, ridgeline, ridgeline_ratio
...
column_dict_dropdown_options = [{'label': column, 'value': column} for column in column_dict.keys()] + [{'label': 'All', 'value': 'all'}]
...
# user defined inputs (bellow values are default)
fig_ratio = ridgeline_ratio(data,
                            year='2022',
                            company_name1="엘지에너지솔루션(연결)",
                            company_name2="삼성SDI(연결)",
...
column_dict_dropdown_options2 = [{'label': column, 'value': column} for column in column_dict.keys()] + [{'label': 'All', 'value': 'all'}]
...
def update_graph(year, column_dict_key, log,):
