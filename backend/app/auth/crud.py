...
def login_user(user: User, db: Session):
    query = text(
        """
            ...
            FROM dev_test.users_security_info
            WHERE user_id = (
                SELECT user_id FROM dev_test.users WHERE id = :id
            )
            """
    )
...
def register_user(profile_info: ProfileInfo, db: Session):
    try:
        ...
        query_2 = text(
            """
            INSERT INTO dev_test.users_profile_info (user_id, address, phone_number)
        ...
        query_3 = text(
            """
            INSERT INTO dev_test.users_security_info (user_id, password, salt)
    ...
    try:
        ...
        if security_info:
            ...
            if hashed_input_password.decode() == stored_password:
                query = text(
                    """
                        DELETE FROM dev_test.users WHERE user_id = :user_id
                        """
                )
